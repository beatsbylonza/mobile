...
export default function Profile({ navigation, route }) {
    ...
    const [firstName, setFirstName] = useState(currentUser.currentUser.name.firstName);
    const [middleName, setMiddleName] = useState(currentUser.currentUser.name.middleName);
    const [lastName, setLastName] = useState(currentUser.currentUser.name.lastName);

    const [email, setEmail] = useState(currentUser.currentUser.email);
    
    const [contactNumber, setContactNumber] = useState(currentUser.currentUser.contactNumber);

    const [street, setStreet] = useState(currentUser.currentUser.address.street);
    const [city, setCity] = useState(currentUser.currentUser.address.city);
    const [state, setState] = useState(currentUser.currentUser.address.state);
    const [zipCode, setZipCode] = useState(currentUser.currentUser.address.zipCode);

    const saveChanges = () =>{
        if(firstName && middleName && lastName && email && street && city && state && zipCode){
            currentUser.currentUser.name.firstName = firstName;
            currentUser.currentUser.name.middleName = middleName;
            currentUser.currentUser.name.lastName = lastName;
            currentUser.currentUser.email = email;
            currentUser.currentUser.contactNumber = contactNumber;
            currentUser.currentUser.address.street = street;
            currentUser.currentUser.address.city = city;
            currentUser.currentUser.address.state = state;
            currentUser.currentUser.address.zipCode = zipCode;

            db.collection('users').doc(currentUser.id).update(currentUser.currentUser).then(()=>{
                Alert.alert(
